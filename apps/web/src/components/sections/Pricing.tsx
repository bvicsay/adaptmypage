        ...
        <SectionHeader
          ...
          title={
            <>
              Open source.
              <br />
              <span className="serif-italic">Free.</span>
            </>
          }
          lede="Bring your own Jev key and there is nothing to pay us. A judgment costs about $0.00007 at Jev’s list price."
        />
        <div className="grid gap-4 self-center">
          {[
            ["SDK", "$0", "MIT. Hooks, server handler, live panel.", "Read the docs", "#docs"],
            ...
            <div key={k} className="grid grid-cols-[110px_1fr_auto] items-baseline gap-4 border-t border-ink/15 pt-4">
              ...
              <div>
                <div className="display text-[1.6rem]">{price}</div>
                <p className="mt-1 text-[0.9rem] text-ink-2">{d}</p>
              </div>
              <a href={href} className="link-arrow !text-[0.85rem]" data-intent={`Pricing: ${cta}`}>
                {cta}
              </a>
            </div>
          ))}
